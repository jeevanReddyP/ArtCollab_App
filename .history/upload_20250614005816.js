...
const inputArea = document.getElementById('input-area'); // container div in your HTML

artSelect.addEventListener('change', () => {
  inputArea.innerHTML = ''; // clear previous inputs

  if (artSelect.value === 'image' || artSelect.value === 'audio') {
    const fileInput = document.createElement('input');
    fileInput.type = 'file';
    fileInput.name = 'artfile';
    fileInput.accept = artSelect.value === 'image' ? 'image/*' : 'audio/*';
    inputArea.appendChild(fileInput);
  } else if (artSelect.value === 'text') {
    const textArea = document.createElement('textarea');
    textArea.name = 'arttext';
    textArea.rows = 6;
    textArea.placeholder = 'Write your poetry or story here...';
...
const uploadWidget = cloudinary.createUploadWidget({
  ...
  uploadPreset: "your_unsigned_preset" // Replace with your unsigned upload preset
}, (error, result) => {
